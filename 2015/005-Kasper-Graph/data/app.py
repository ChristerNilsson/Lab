...
def plot_complexity_curves(ccurves, coverage=0, read_length=0, real_counts_path=False, use_unique=True, output_name='complexity_curves', x_min=0, x_max=500000000):
    ...
    for ccurve in ccurves:
        ...
        # Plot the curve
        print ccurve_TOTAL_READS[x_min_ccurve_limit:x_max_ccurve_limit]
        print ccurve_EXPECTED_DISTINCT[x_min_ccurve_limit:x_max_ccurve_limit]
...
#    ax.set_position([0.08, box.y0+0.05, (box.width * 0.78)-0.02, box.height-0.05])
    ...
    ax.legend(legend[0], legend[1],loc='center left', bbox_to_anchor=(0.9, 0.5), prop=font)

    # now save the plot
    png_fn = "{}.png".format(output_name)
    pdf_fn = "{}.pdf".format(output_name)
    pdf_fn = "{}.svg".format(output_name)
    plt.savefig(png_fn)
    plt.savefig(pdf_fn)
    plt.close(fig)
    return 0
...
if __name__ == '__main__':
    ...
    ccurves = ['yield_ACACGC.preseq','yield_CTATAC.preseq','yield_GATCTC.preseq','yield_TACTAG.preseq','yield_TCAGAC.preseq','yield_TGTGTA.preseq'] #

    plot_complexity_curves(ccurves = ccurves, coverage=1000, read_length=1000, x_max = 500000)