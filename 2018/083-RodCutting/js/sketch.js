...
var Button, buttons, cut_rod, draw, execute, lst, mousePressed, parts, prices, setup, showInfo;
...
setup = function setup() {
  ...
  fn = function fn(i) {
    buttons.push(new Button(i + 1, 110 + 50 * i, 20, function () {
      if (prices[i] > 1) {
        prices[i]--;
      }
    ...
    return buttons.push(new Button(price, 110 + 50 * i, 50, function () {
      prices[i]++;
      buttons[2 * i + 1].title = prices[i];
      return execute();
    }));
...
draw = function draw() {
  var button, i, index, l, len, len1, len2, n, o, price, ref, results;
  bg(0.5);
  ...
  textAlign(LEFT, CENTER);
  ref = range(30).concat(range(9990, 10000));
  ...
  textAlign(CENTER, CENTER);
  results = [];
  for (i = o = 0, len2 = prices.length; o < len2; i = ++o) {
    price = prices[i];
    results.push(showInfo(i));
  }
  return results;
};
