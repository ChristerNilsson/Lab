...
var A, B, C, D, E, FILENAME, HEIGHT, SCALE, TRACKED, WIDTH, buttons, corner, cx, cy, draw, drawButtons, drawPoints, drawTrack, gps, hortal, img, locationUpdate, locationUpdateFail, makeCorners, mousePressed, mouseReleased, myround, points, position, preload, released, setup, show, spara, track, vercal;
...
setup = function setup() {
  ...
  y1 = 100;
  y2 = height - 100;
  buttons.push(new Button('save', x1, y1, function () {
    return points.push(position);
  }));
  buttons.push(new Button('up', x, y1, function () {
    return cy -= 0.5 * height / SCALE;
  }));
  buttons.push(new Button('drop', x2, y1, function () {
    if (points.length > 0) {
      return points.pop();
    }
  }));
