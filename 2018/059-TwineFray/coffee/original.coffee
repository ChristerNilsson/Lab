...
events.Market = ->
	if person.coins >= POTION then link "Potion", "Buy A Healing Potion For #{POTION} Coins"
	else Black "Healing potions cost #{POTION}, but you only have #{person.coins}"
	if person.coins >= SWORD then link "Sword", "Buy A Sword For #{SWORD} Coins"
...
events.useWeapon = ->
	link 'Punch'
	link 'Kick'
	if person.sword > 0 then link 'useSword','Use Sword'
	if person.ax > 0 then link "Use Ax"
