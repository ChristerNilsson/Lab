...
# Tid i Sampers gammal kod: 82 minuter. Orsak: lste 4000GB
...
# for i in range(6):
# 	for j in range(2):
# 		totDel[aggregate[i]] += inMatrices[i][j]
#
# for i, p in enumerate(purposes):
# 	ai = aggregate[i]
# 	for j, cg in enumerate(cgs):
# 		moutBin = inMatrices[i][j] * mulMatrices[ai]
# 		kvot = np.where(totDel[ai] > 0, inMatrices[i][j]/totDel[ai], 1.0 / totDelCount[ai])
# 		moutBin += addMatrices[ai] * kvot
# 		moutBin.astype(dt).tofile("R125 CashCard/Kollektivt" + p + ' ' + cg + "_NewAdj.bin")
#
