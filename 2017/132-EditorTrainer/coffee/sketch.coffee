...
PROBLEMS = [ # operations,line,ch,startläge,slutläge
	[1,2,8,ENG, "#{A}#{B}\t\tx = lezrp 10,30,i\n#{D}#{E}"] # z
	[1,2,8,ENG, "#{A}#{B}\t\tx = lrp 10,30,i\n#{D}#{E}"] # Backspace
	[1,2,8,ENG, "#{A}#{B}\t\tx = lep 10,30,i\n#{D}#{E}"] # Del
	[1,2,8,ENG, "#{A}#{B}#{D}#{E}"] # CtrlX
	[1,2,8,ENG, "#{A}#{B}#{C}#{C}#{D}#{E}"] # ctrl-shift-D
	[1,2,8,ENG, "#{A}#{B}#{C}\t\t\n#{D}#{E}"] # ctrl-Enter
	[1,2,8,ENG, "#{A}#{B}\t\t\n#{C}#{D}#{E}"] # ctrl-shift-Enter
	[1,2,8,ENG, "#{A}#{C}#{B}#{D}#{E}"] # ctrl-shift-Up
	[1,2,8,ENG, "#{A}#{B}#{D}#{C}#{E}"] # ctrl-shift-Down
	[1,2,8,ENG, "#{A}#{B}\t\tx = le\n#{D}#{E}"] # ctrl-KK
	[1,2,8,ENG, "#{A}#{B}\tx = lerp 10,30,i\n#{D}#{E}"] # shift-Tab
	[2,2,8,ENG, "#{A}#{B}\t#{C}#{D}#{E}"] # Home Tab
	[2,2,8,ENG, "z#{A}#{B}#{C}#{D}#{E}"] # ctrlHome z
	[2,2,8,ENG, "#{A}#{B}\t\tx = le 10,30,i\n#{D}#{E}"] # Del Del
	[2,2,8,ENG, "#{A}#{B}#{C}#{D}#{E}z"] # ctrlEnd z
	[2,2,8,ENG, "#{A}#{B}\t\tx = lerp 10,30,iz\n#{D}#{E}"] # End z
	[2,2,8,ENG, ""] # ctrlA Del
	[2,2,8,ENG, 'for i in range 10\nfor j in range 10\n\tx = lerp 10,30,i\n\ty = lerp 10,30,j\n\trect x,y,10,10\n'] # ctrlA shiftTab
	[2,2,8,ENG, "\t#{A}\t#{B}\t#{C}\t#{D}\t#{E}"] # ctrlA Tab
	[2,2,8,ENG, "#{A}#{B}\t\tx =  10,30,i\n#{D}#{E}"] # ctrl-D Del
	[3,2,8,ENG, "#{A}#{B}#{C}\t\ty = lerp 10,30,jz\n#{E}"] # Down End z
	[3,2,8,ENG, "for i in range 10z\n\t#{B}#{C}#{D}#{E}"] # ctrlHome End z
	[3,2,8,ENG, "#{A}#{B}z#{C}#{D}#{E}"] # Home Home z
	[4,2,8,ENG, "#{A}#{B}\t\tx = rple 10,30,i\n#{D}#{E}"] # ctrlShiftRight ctrlX Home paste
	[5,2,8,ENG, "#{A}#{B}\t\tx = lerplerplerp 10,30,i\n#{D}#{E}"] # ctrlD ctrlC ctrlV ctrlV ctrlV
	[5,2,8,ENG, 'for i in range 10 for j in range 10 x = lerp 10,30,i y = lerp 10,30,j rect x,y,10,10\n'] # ctrl-Home ctrl-J ctrl-J ctrl-J ctrl-J
	[5,2,8,ENG, "#{A}#{B}#{C}#{D}#{C}#{D}#{E}"] # ctrl-L ctrl-L ctrl-C ctrl-V ctrl-V
] 
